        ...
        res.on('end', () => {
          ...
          if (res.statusCode < 200 || res.statusCode > 300) {
            return reject(
              Object.assign(
                new Error(
                  `Invalid status code '${res.statusCode}' for url '${url}'`
                ),
                { res, body }
              )
            )
...
const trimLeft = (value, charlist = '/') =>
  value.replace(new RegExp(`^[${charlist}]*`), '')
const trimRight = (value, charlist = '/') =>
  value.replace(new RegExp(`[${charlist}]*$`), '')
...
const main = async () => {
  ...
  if (!branch) {
    const headers = {
      'User-Agent': 'github.com/chendj001/githubPushAction'
    }
    if (process.env.INPUT_GITHUB_TOKEN)
      headers.Authorization = `token ${process.env.INPUT_GITHUB_TOKEN}`
